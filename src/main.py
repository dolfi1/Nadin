...
SOURCE_PROVIDERS: list[dict[str, Any]] = [
    ...
    {"name": "checko.ru", "kind": "checko", "url_template": "https://checko.ru/search?query={query}", "supports_inn": True, "supports_name": True, "supports_url": True, "is_person_source": True, "priority": 6},
...
INTERNAL_MERGED_SOURCE = "__merged__"
...
class CompanyWebApp:
    ...
    def _apply_proxy_to_session(self) -> None:

        proxies = self._get_proxy_settings()

        if proxies:

            self._http_session.proxies = proxies

            logger.info("Proxy applied to HTTP session: %s", proxies)
    ...
    def reload_proxy_settings(self) -> None:

        self._apply_proxy_to_session()

        self._cloudscraper_session = None

        if hasattr(self, '_last_fetch_status_by_domain'):

            self._last_fetch_status_by_domain.clear()

        logger.info("Proxy settings reloaded, fetch cache cleared")
    ...
    def _fetch_from_provider(
        ...
        if kind in {"wikipedia_html", "duckduckgo_html"}:
            ...
            profile = self._parse_generic_osint(url, provider.get("name", kind))
            if profile:
                return profile
            if kind == "checko":
                return self._parse_from_ddg_site(raw, ["checko.ru"], provider.get("name", kind))
        ...
        if kind == "zachestnyibiznes_scrape":
            ...
            profile = self._parse_zachestnyibiznes_direct(raw)
            if profile:
                return profile
            return self._parse_from_ddg_site(raw, ["zachestnyibiznes.ru"], provider.get("name", kind))
        ...
        if kind == "rbc_companies_scrape":
            ...
            profile = self._parse_rbc_companies_direct(raw)
            if profile:
                return profile
        ...
        if kind == "kontur":
            ...
            profile = self._parse_kontur(raw)
            if profile:
                return profile
            return self._parse_from_ddg_site(raw, ["focus.kontur.ru"], provider.get("name", kind))
        ...
        if kind in {"bank_russia", "rbc"}:
            ...
            if profile:
                return profile
            return self._parse_from_ddg_site(raw, ["checko.ru"], provider.get("name", kind)) if kind == "checko" else profile
        ...
        if kind in {"google_search", "yandex_search", "linkedin_search", "facebook_search", "offshoreleaks", "checko", "zachestnyibiznes", "sherlock", "maigret", "holehe", "theharvester"}:
            ...
            if kind == "checko":
                ...
                if inn:
                    checko_profile, checko_status, _checko_reason = self._fetch_from_checko(raw, inn=inn)
                    if checko_profile is not None:
                        return checko_profile
                    if checko_status == "rate_limited":
                        return []

                template = provider.get("url_template", "") or "https://checko.ru/search?query={query}"
    ...
    def _add_osint_providers(self) -> None:
        ...
        osint_providers = [
            ...
            {"name": "checko.ru", "kind": "checko", "url_template": "https://checko.ru/search?query={query}", "supports_inn": True, "supports_name": True, "priority": 50},
    ...
    def _search_external_sources(
        ...
        def load_provider(provider: dict[str, Any]) -> tuple[str, list[dict[str, Any]], str]:
            ...
            try:
                ...
                data = self._call_provider(provider, raw, input_type, no_cache=no_cache, search_type=search_type, allow_fallback=True)
        ...
        try:
            ...
            trace.append("hits_by_provider: " + ", ".join(f"{k}={v}" for k, v in hits_by_provider.items() if k not in {"Scrapy Merge", INTERNAL_MERGED_SOURCE}))
            ...
            if os.getenv("SCRAPY_PIPELINE_MERGE", "1").lower() in {"1", "true", "yes"} and hits:
                ...
                if merged:
                    ...
                    if merged_hit is not None:
                        ...
                        # Scrapy Merge используется внутри, не показываем в trace
    ...
    def _get_proxy_settings(self) -> dict | None:
...
"""
        ...
        *,
        ...
        if selector is None:
            ...
            for anchor in soup.find_all("a", href=True):
                href = self._normalize_spaces(str(anchor.get("href", "")))
                if search_type == "company" and not href.startswith("/id/"):
                    continue
                if search_type == "person" and not href.startswith("/person/"):
                    continue
                if not search_type and not (href.startswith("/id/") or href.startswith("/person/")):
                    continue
                name = self._normalize_spaces(anchor.get_text(" ", strip=True))
                if not name:
                    continue
                parent = anchor.parent
                block_text = self._normalize_spaces(parent.get_text(" ", strip=True)) if parent is not None else ""
                inn_pattern = r"\b(\d{10}|\d{12})\b" if href.startswith("/person/") else r"\b(\d{10})\b"
                inn_match = re.search(inn_pattern, block_text)
                hit_type = "person" if href.startswith("/person/") else "company"
                hits.append({
    ...
    def _pick_primary_card_source(
        ...
        for key in ("ru_org", "inn", "surname_ru", "name_ru"):
            ...
            if source_name in {"Scrapy Merge", INTERNAL_MERGED_SOURCE}:
                continue
        ...
        for hit in source_hits:
            ...
            if source_name and source_name not in {"Scrapy Merge", INTERNAL_MERGED_SOURCE}:
    ...
    def _search_by_company(self, company_name: str, search_type: str = "") -> tuple[list[dict[str, Any]], list[str]]:
        ...
        if short_brand_query:
            ...
            if SHORT_BRAND_KNOWN_INN.get(brand_token, ""):
                ...
                quick_provider_names = ["ФНС ЕГРЮЛ", "zachestnyibiznes.ru", "companies.rbc.ru", "focus.kontur.ru", "rusprofile.ru"]


                trace.append("Short-brand quick providers: \u0424\u041d\u0421 \u0415\u0413\u0420\u042e\u041b, zachestnyibiznes.ru, companies.rbc.ru, focus.kontur.ru, rusprofile.ru")
        ...
        hits: list[dict[str, Any]] = []
        known_brand_inn = SHORT_BRAND_KNOWN_INN.get(brand_token, "") if brand_token else ""
        known_brand_provider_names = ["ФНС ЕГРЮЛ", "zachestnyibiznes.ru", "companies.rbc.ru", "focus.kontur.ru", "rusprofile.ru", "checko.ru"]

        if known_brand_inn:
            trace.append(f"Short-brand seed INN: {known_brand_inn}")
            seed_hits, seed_trace = self._search_external_sources(
                known_brand_inn,
                no_cache=False,
                search_type="company",
                provider_names=known_brand_provider_names,
            )
            trace.extend(seed_trace)
            hits.extend(seed_hits)
        ...
        if known_brand_inn and not hits:
            trace.append(f"Short-brand fallback by INN: {known_brand_inn}")
            known_inn_hits, known_inn_trace = self._search_external_sources(
                known_brand_inn,
                no_cache=False,
                search_type="company",
                provider_names=known_brand_provider_names,
            )
            trace.extend(known_inn_trace)
            if known_inn_hits:
                hits.extend(known_inn_hits)
                hits = self._dedup_source_hits(hits)
    ...
    def _search_by_criteria(self, params: dict[str, str]) -> tuple[list[dict[str, Any]], list[dict[str, str]], list[str]]:
        ...
        if queried_providers:
            ...
            trace.append("Провайдеры в поиске: " + ", ".join(sorted(p for p in queried_providers if p and p not in {"Scrapy Merge", INTERNAL_MERGED_SOURCE})))
